...
import com.twozo.enums.LoanColumn;
...
public class LoanDaoDatabaseImpl implements LoanDao {
	...
	private static LoanDaoDatabaseImpl instance;

	@Override
	public boolean saveLoanById(final String loanId, final Loan loan)
			throws LoanNotEligibleException, DatabaseException {
		if (loan == null) {
			return false;
		}
	...
	@Override
	public final Loan getLoanDetails(final String loanNumber) throws DatabaseException {
		final String query = "SELECT l.*, a.account_number, c.id AS customer_id, c.name, c.gender, c.mobile_number, " +
				"c.aadhar_number, c.pan_card_number, c.dob, c.address, c.photo_url " +
				"FROM loans l " + "JOIN accounts a ON l.account_id = a.id " +
				"JOIN customers c ON a.customer_id = c.id " + "WHERE l.loan_number = ?";
		...
		try (Connection conn = DatabaseConnection.getConnection();
			...
			try (ResultSet rs = statement.executeQuery()) {
				if (rs.next()) {
					generatedLoanId = rs.getLong("id");
					final LoanType loanType = LoanType.valueOf(rs.getString("loan_type"));
					Loan loan;
					...
					case "homeloan":
						...
						fetchHomeLoanDetails(conn, (HomeLoan) loan, generatedLoanId);
						break;
					case "personalloan":
						loan = new PersonalLoan();

						fetchPersonalLoanDetails(conn, (PersonalLoan) loan, generatedLoanId);
						break;
					case "educationloan":
						loan = new EducationLoan();

						fetchEducationLoanDetails(conn, (EducationLoan) loan, generatedLoanId);
						break;
					default:
						loan = new Loan();
						break;
					...
					loan.setLoanNumber(rs.getString(LoanColumn.LOAN_NUMBER.getDescription()));
					loan.setLoanAmount(rs.getDouble(LoanColumn.LOAN_AMOUNT.getDescription()));
					loan.setAccountNumber(rs.getString(LoanColumn.ACCOUNT_NUMBER.getDescription()));
					loan.setLoanType(LoanType.valueOf(rs.getString(LoanColumn.LOAN_TYPE.getDescription())));
					loan.setLoanTenure(LoanTenure.valueOf(rs.getString(LoanColumn.LOAN_TENURE.getDescription())));
					loan.setEmiAmount(rs.getDouble(LoanColumn.EMI_AMOUNT.getDescription()));
					loan.setBalanceAmount(rs.getDouble(LoanColumn.BALANCE_AMOUNT.getDescription()));
					loan.setLoanStatus(LoanStatus.valueOf(rs.getString(LoanColumn.LOAN_STATUS.getDescription())));
					loan.setApplicationDate(rs.getDate(LoanColumn.APPLICATION_DATE.getDescription()).toLocalDate());
					loan.setInterestRate(rs.getDouble(LoanColumn.INTEREST_RATE.getDescription()));
	...
	@Override
	public final boolean removeLoan(final String loanNumber) throws DatabaseException {
		final String query = "DELETE FROM loans WHERE loan_number = ?";
	...
	@Override
	public final boolean repayLoanAmount(final String loanId, final double repaymentAmount)
			throws InsufficientAmount, DatabaseException {
		final String query = "UPDATE loans SET balance_amount = balance_amount - ? WHERE loan_number = ?";
		...
		try (Connection conn = DatabaseConnection.getConnection();
			...
			statement.setDouble(1, repaymentAmount);
	...
	public boolean updateLoanDetails(final String updateData, final String loanId, final String updateType)
			throws DuplicateUpdateException, DatabaseException, InvalidCustomerUpdateException {
	...
	public static final LoanDaoDatabaseImpl getInstance() {
		if (instance == null) {
			instance = new LoanDaoDatabaseImpl();
		}
		return instance;
	}
}